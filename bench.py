...
openai.api_key = "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"# open(os.path.expanduser('~/.openai')).read().strip()

CLUSTER_COUNT = 10
QUOTE_DATASET_COUNT = 1178
...
def get_label(cluster, labels, quotes):
...
def run_embedding_test(engine):
    ...
    for quote in tqdm.tqdm(quotes):
        embeddings.append(get_embeddings(quote, engine))
    embeddings = np.array(embeddings)
    print("Clustering...")
    # What if I change this to support vector machine?
    ...
    for cluster in tqdm.tqdm(range(CLUSTER_COUNT)):
        label = get_label(cluster, labels, quotes)
        labels_dict[cluster] = label
    print(labels_dict)
    # quote_ids = [str(i) for i in quotes.index]
    quote_ids = [str(i) for i, quote in enumerate(quotes)]
    hashbit = hashlib.sha256("".join(quote_ids).encode()).hexdigest()[-5:]
    engine_filename = engine.replace("/", "-")
    file_name = f'{hashbit}-{engine_filename}-cluster{CLUSTER_COUNT}-quote{QUOTE_DATASET_COUNT}'
